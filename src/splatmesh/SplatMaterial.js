...
export class SplatMaterial  {
    ...
    vertexShaderSource += `
    ${customVars}
    uniform vec3 splatColor; // Новый uniform для цвета
    uniform float hue;
    uniform vec2 focal;
    ...
    mat3 calculateHueShiftMatrix(float angle) {
        float c = cos(angle);
        float s = sin(angle);
        return mat3(
    ...
    void main () {
        ...
        // Новый код для обработки цвета
        vec3 baseColor = uintBitsToFloat(uvec3(sampledCenterColor.rgb)); // Преобразование цвета
        mat3 hueShiftMatrix = calculateHueShiftMatrix(hue); // Матрица сдвига оттенка
        vec3 adjustedColor = hueShiftMatrix * baseColor; // Применяем hue shift
        vColor = vec4(adjustedColor, 1.0); // Устанавливаем цвет для использования в фрагментном шейдере

        gl_Position = projectionMatrix * modelViewMatrix * vec4(splatCenter, 1.0); // Итоговое положение
    }
`;
...
`;
    ...
    static getUniforms(dynamicMode = false, enableOptionalEffects = false, maxSphericalHarmonicsDegree = 0,
        ...
        const uniforms = {
            ...
            'hue': { // Добавляем новый uniform
            'type': 'f',
            'value': 0.0 // Начальное значение hue (например, красный)
            },
            'splatColor': { // Еще один пример, если нужен цвет в RGB
            'type': 'v3',
            'value': new THREE.Color(1.0, 0.0, 0.0) // Начальный цвет
            },
            'fadeInComplete': {
                'type': 'i',
                'value': 0
